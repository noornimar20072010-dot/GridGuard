    ...
    <div className="space-y-6">
      ...
      <div className="relative rounded-lg overflow-hidden">
        ...
        <div className="relative p-8 text-center">
          <div className="absolute top-4 left-4 text-xs text-slate-200 font-mono">
          ...
          <p className="text-slate-100 text-sm">Real-time electrical distribution network monitoring</p>

          <div className="absolute top-4 right-4 text-xs text-slate-200 font-mono">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="rounded-lg bg-slate-800/50 border border-slate-700/30 p-4">
          <p className="text-xs text-slate-200 uppercase tracking-wider font-semibold">Avg Load</p>
          <p className="text-3xl font-bold text-white mt-2">{totalLoad}%</p>
        </div>
        <div className="rounded-lg bg-slate-800/50 border border-slate-700/30 p-4">
          <p className="text-xs text-slate-200 uppercase tracking-wider font-semibold">Critical</p>
          <p className="text-3xl font-bold text-red-400 mt-2">{critCount}</p>
        </div>
        <div className="rounded-lg bg-slate-800/50 border border-slate-700/30 p-4">
          <p className="text-xs text-slate-200 uppercase tracking-wider font-semibold">Warning</p>
          <p className="text-3xl font-bold text-amber-400 mt-2">{warnCount}</p>
        </div>
        <div className="rounded-lg bg-slate-800/50 border border-slate-700/30 p-4">
          <p className="text-xs text-slate-200 uppercase tracking-wider font-semibold">Normal</p>
          ...
          <div className="relative border-b border-slate-700/50 p-6 flex items-center justify-between bg-slate-800/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-200 mt-1">Interactive grid distribution visualization</p>
              </div>
            </div>
  ...
  return (
    ...
    >
      ...
      {isElevated && predictedLoad != null && (
        <span className="shrink-0 text-sm text-slate-200">→ {predictedLoad}%</span>
      )}
    </Link>
  )
}
...
function TreeToggle({
  ...
  return (
    ...
    >
      <ChevronRight
        className={cn('h-3.5 w-3.5 shrink-0 text-slate-200 transition-transform', isExpanded && 'rotate-90')}
      />
      {icon}
      <span className={cn('truncate', labelClassName)}>{label}</span>
      <span className="ml-auto shrink-0 text-sm text-slate-200">{meta}</span>
    </button>
  )
}
  ...
  return (
    ...
    >
      ...
      <svg width={svgWidth} height={svgHeight} className="absolute inset-0 opacity-20 pointer-events-none" viewBox="0 0 1200 600">
        ...
        {/* Map text label */}
        <text x="50" y="40" fontSize="18" fill="white" opacity="0.6" fontWeight="bold">
          DELHI GRID MAP REFERENCE
        </text>
        <text x="50" y="65" fontSize="12" fill="white" opacity="0.5">
            ...
            >
              ...
              {isTransformer ? (
                ...
                <text
                  ...
                  fill={nodeColor}
                  style={{ pointerEvents: 'none', fontWeight: 'bold', fill: nodeColor, color: nodeColor }}
                >
              ...
              {isTransformer && (
                <text
                  ...
                  className="text-sm font-bold"
                  fill={nodeColor}
                  style={{ pointerEvents: 'none', fontWeight: 'bold', fontSize: '12px', fill: nodeColor, color: nodeColor }}
                >
                  {node.label}
                </text>
              )}
      ...
      <div className="flex flex-wrap gap-6 mt-4 justify-center text-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-200">Normal (Green)</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-amber-500" />
          <span className="text-slate-200">Warning (Amber)</span>
        </div>
        <div className="flex items-center gap-2">
          <svg width="12" height="12" viewBox="0 0 12 12">
            <line x1="0" y1="6" x2="12" y2="6" stroke="#ef4444" strokeWidth="2" strokeDasharray="2,2" />
          </svg>
          <span className="text-slate-200">Critical (Red Dashed)</span>
        </div>
      </div>
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-lg p-6 max-w-md w-full shadow-2xl">
            <div className="flex justify-between items-start mb-4">
              ...
              <button
                ...
                className="text-slate-200 hover:text-slate-100 transition"
              >
                ✕
              </button>
            </div>
...
function DetailRow({ label, value, highlight = false, fontSize = 'text-sm' }: { label: string; value: string; highlight?: boolean; fontSize?: string }) {
  return (
    <div className="flex justify-between items-center">
      <span className={`text-slate-200 ${fontSize}`}>{label}</span>
